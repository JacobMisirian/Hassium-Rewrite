...
namespace Hassium.Runtime.Util
{
    public class HassiumOS : HassiumObject
    {
        ...
        public HassiumProcess exec(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
        ...
        public HassiumNull exit(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
            Environment.Exit(args.Length == 0 ? 0 : (int)args[0].ToInt(vm, location).Int);
            return Null;
        }

        [FunctionAttribute("exitcode { get; }")]
        public HassiumInt get_exitcode(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
            return new HassiumInt(Environment.ExitCode);
        }

        [FunctionAttribute("exitcode { set; }")]
        public HassiumInt set_exitcode(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
            Environment.ExitCode = (int)args[0].ToInt(vm, location).Int;
            return args[0].ToInt(vm, location);
        }

        [FunctionAttribute("func getarg (arg : string) : string")]
        public HassiumString getarg(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
            return new HassiumString(Environment.GetEnvironmentVariable(args[0].ToString(vm, location).String));
        }

        [FunctionAttribute("func getargs () : list")]
        public HassiumList getargs(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
            HassiumList arguments = new HassiumList(new HassiumObject[0]);

            foreach (var arg in Environment.GetCommandLineArgs())
                arguments.add(vm, location, new HassiumString(arg));

            return arguments;
        }

        [FunctionAttribute("machinename { get; }")]
        public HassiumString get_machinename(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
            return new HassiumString(Environment.MachineName);
        }

        [FunctionAttribute("netversion { get; }")]
        public HassiumString get_netversion(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
            return new HassiumString(Environment.Version.ToString());
        }

        [FunctionAttribute("newline { get; }")]
        public HassiumString get_newline(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
            return new HassiumString(Environment.NewLine);
        }

        [FunctionAttribute("func setarg (arg : string, val : string) : null")]
        public HassiumNull setarg(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
            Environment.SetEnvironmentVariable(args[0].ToString(vm, location).String, args[1].ToString(vm, location).String);
            return Null;
        }
        ...
        public HassiumString get_version(VirtualMachine vm, SourceLocation location, params HassiumObject[] args)
        {
